...
class DCT_base_Rec_Module(nn.Module):
    ...
    def forward(self, x):
        ...
        weighted = _x * self.learnable_weights
        grade = weighted.sum(dim=(2,3,4))

        max_vals, max_idx = torch.topk(grade, N, dim=1)

        min_vals, min_idx = torch.topk(grade, N, dim=1, largest=False)

        maxmax_idx  = max_idx[:, 0]
        maxmax_idx1 = max_idx[:, 1] if N > 1 else max_idx[:, 0]

        minmin_idx  = min_idx[:, 0]
        minmin_idx1 = min_idx[:, 1] if N > 1 else min_idx[:, 0]

        batch_idx = torch.arange(B, device=level_x_unfold.device)

        x_minmin  = level_x_unfold[batch_idx, minmin_idx]
        x_maxmax  = level_x_unfold[batch_idx, maxmax_idx]
        x_minmin1 = level_x_unfold[batch_idx, minmin_idx1]
        x_maxmax1 = level_x_unfold[batch_idx, maxmax_idx1]
